...
document.addEventListener("DOMContentLoaded", function () {
	...
	fetch('get_monedero.php')
		.then(response => response.json())
		.then(data => {
			bankValue = data.monedero;
			document.getElementById('monedero').innerText = bankValue; 
			startGame();
		})
		.catch(error => console.error('Error fetching monedero:', error));
});

function resetGame() {
	fetch('reset_monedero.php')
		.then(response => response.json())
		.then(data => {
			bankValue = data.monedero;
			currentBet = 0;
...
function buildBettingBoard() {
	...
	let numberBlocks = [3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, '2 to 1', 2, 5, 8, 11, 14, 17, 20, 23, 26, 29, 32, 35, '2 to 1', 1, 4, 7, 10, 13, 16, 19, 22, 25, 28, 31, 34, '2 to 1'];
